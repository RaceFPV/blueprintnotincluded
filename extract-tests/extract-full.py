...
def extract_ui_sprites(database):
    """Finds all '_ui' sprite metadata from uiSprites."""
    return [sprite for sprite in database.get("uiSprites", []) if "_ui" in sprite.get("name", "")]
...
def process_all_sprites(database_path, image_folder, output_folder):
    """Processes all UI sprites and extracts them from their respective image files."""
...
if __name__ == "__main__":
    ...
    output_folder = "output"  # Folder to store extracted sprites
