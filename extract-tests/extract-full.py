...
def process_textures(database_path, image_folder, output_folder):
    ...
    try:
        ...
        # Get unique texture names from buildings and sprites
        texture_names = set()
        ...
        # From sprites (in case there are any not in buildings)
        for sprite in database.get("uiSprites", []):
            if sprite.get("textureName"):
                texture_names.add(sprite["textureName"])
        
        print(f"\nFound {len(texture_names)} unique textures to process")
        ...
        for texture_name in sorted(texture_names):
            try:
                ...
                if not sprites:
                    print(f"No sprites found for texture: {texture_name}")
                    continue
                
                print(f"\nProcessing {texture_name} ({len(sprites)} sprites)")
                composite_texture_sprites(image_path, sprites, output_path, database)
                
            except Exception as e:
                print(f"Error processing texture {texture_name}: {str(e)}")
                continue
